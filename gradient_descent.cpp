...
int main(int argv, char **argc) {
  ...
  if (argv >= 2) numberOfDimensions = (unsigned int)atoi(argc[1]);
  if (argv >= 3) numberOfParticles = (unsigned int)atoi(argc[2]);
  if (argv >= 4) numberOfMetropolisSteps = (unsigned int)pow(10, atof(argc[3]));
  if (argv >= 5)
    numberOfEquilibrationSteps = (unsigned int)pow(10, atof(argc[4]));
  if (argv >= 6) omega = (double)atof(argc[5]);
  if (argv >= 7) alpha = (double)atof(argc[6]);
